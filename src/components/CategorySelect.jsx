...
export function CategorySelect({ categories, value, onChange, onCategoryCreated }) {
  ...
  const handleCreate = async () => {
    ...
    setError(null);
    try {
      const category = await adminApi.createCategory({ category_name: newName.trim() });
    ...
    return (
      <div className="flex items-center gap-1.5">
        <input
          ...
          className="bg-[#141210] border border-[#C9A24B] rounded-md px-2.5 py-1.5 text-xs text-[#EDE7DD] focus:outline-none w-32"
        />
        <button
          type="button"
